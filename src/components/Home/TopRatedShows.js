...
export default function TopRatedShows() {
  ...
  async function getTopRatedShows() {
    const response = await fetch(
      `https://api.themoviedb.org/3/tv/top_rated?api_key=${process.env.REACT_APP_MOVIEDB_API_KEY}&language=en-US&page=1`
    );
    const data = await response.json();
    setTopRatedShows(
      data.results.filter((data) => data.backdrop_path != null).slice(0, 12)
    );
  }
      ...
      {topRatedShows && (
        ...
        >
          {topRatedShows.map((show, i) => (
            <Grid key={i} item xs={4} sm={4} md={4} className="top-rated-card">
              {currentUser ? (
                <Link
                  to={`/detail/tv/${show.id}/${currentUser && currentUser.uid}`}
                  underline="none"
                >
