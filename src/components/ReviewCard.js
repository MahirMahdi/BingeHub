...
export default function ReviewCard({type_id,user_id,content, image, author}){
    const [currentUser] = useContext(AuthContext);
    const reviewRef = doc(db, "users", "reviews");

    async function handleRemove(){
        const delete_doc = {author: author, author_id: currentUser.uid, image: image, review: content, type_id: type_id};
    ...
    return(
        <Card className="review-card" sx={{backgroundColor:"black"}}>
            <Box className="author-info">
                ...
                {currentUser && user_id === currentUser.uid?<Button onClick={handleRemove} className="r-remove-button" variant="contained" color="error">X</Button>:null}
            </Box>
